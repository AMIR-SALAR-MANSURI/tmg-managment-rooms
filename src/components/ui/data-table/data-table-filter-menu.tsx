...
import type { ExtendedColumnFilter, FilterOperator } from "./table-meta";
...
export function DataTableFilterMenu<TData>({
  ...
  React.useEffect(() => {
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (
        ...
        event.preventDefault();
        onFilterRemove((filters[filters.length - 1]?.filterId ?? "") as string);
      }
    }
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {filters.map((filter) => (
        <DataTableFilterItem
          key={filter.filterId as React.Key}
      ...
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <Button
            ...
            className={cn(filters.length > 0 && "size-8")}
            ref={triggerRef}
            onKeyDown={onTriggerKeyDown}
          >
            <ListFilter />
            {filters.length > 0 ? null : "فیلتر"}
          </Button>
        </PopoverTrigger>
        <PopoverContent
          align={align}
          ...
          <Command loop className="[&_[cmdk-input-wrapper]_svg]:hidden">
            ...
            <CommandList>
              {selectedColumn ? (
                ...
                <>
                  <CommandEmpty>فیلدی یافت نشد</CommandEmpty>
                  <CommandGroup>
                    {columns.map((column) => (
                      <CommandItem
                        key={column.id}
...
}: DataTableFilterItemProps<TData>) {
  {
    ...
    return (
      <div
        ...
        role="listitem"
        id={filterItemId}
        className="flex flex-row-reverse items-center rounded-md bg-background"
      ...
      >
        <Popover open={showFieldSelector} onOpenChange={setShowFieldSelector}>
          <PopoverTrigger asChild>
            <Button
              variant="ghost"
              size="sm"
              className="rounded-none h-10 rounded-l-md border border-r-0 font-normal dark:bg-input/30"
            >
              {columnMeta?.icon && (
          ...
          >
            <Command loop>
              <CommandInput placeholder="جستجوی فیلد ها" />
              <CommandList>
                <CommandEmpty>فیلدی یافت نشد.</CommandEmpty>
                <CommandGroup>
                  {columns.map((column) => (
        ...
        <Button
          ...
          variant="ghost"
          size="sm"
          className="h-10 rounded-none rounded-r-md border border-l-0 px-1.5 font-normal dark:bg-input/30"
...
function FilterValueSelector<TData>({
  ...
  const variant: any = column.columnDef.meta?.variant ?? "text";
        ...
        <CommandGroup>
          {column.columnDef.meta?.options?.map((option) => (
            <CommandItem
              ...
              onSelect={() => onSelect(option.value as any)}
            >
        ...
        <CommandGroup>
          ...
          >
            {isEmpty ? (
              <>
                <Text />
                <span>برای فیلتر کردن بنویسید ...</span>
              </>
            ) : (
              <>
                <BadgeCheck />
  ...
  if (filter.operator === "isEmpty" || filter.operator === "isNotEmpty") {
    return (
      <div
        ...
        aria-live="polite"
        className="h-10 w-16 rounded-none border bg-transparent px-1.5 py-0.5 text-muted-foreground dark:bg-input/30"
      />
    );
  }
    ...
    case "range": {
      if (
        ...
        return (
          <DataTableRangeFilter
            ...
            inputId={inputId}
            onFilterUpdate={onFilterUpdate}
            className="size-full max-w-28 h-10 gap-0 [&_[data-slot='range-min']]:border-r-0 [&_input]:rounded-none [&_input]:px-1.5"
          />
        );
      }
      ...
      return (
        <Input
          ...
          placeholder={
            column.columnDef.meta?.placeholder ?? "مقدار را وارد کنید."
          }
          className="h-10 w-28 rounded-none px-1.5"
      ...
      return (
        ...
        >
          <SelectTrigger
            ...
            className="rounded-none bg-transparent h-10 px-1.5 py-0.5 [&_svg]:hidden"
          >
        ...
        <Popover open={showValueSelector} onOpenChange={setShowValueSelector}>
          <PopoverTrigger asChild>
            <Button
              ...
              variant="ghost"
              size="sm"
              className="h-10 rounded-none border px-1.5 font-normal dark:bg-input/30"
            >
              {selectedOptions.length === 0 ? (
                ...
                <>
                  {selectedOptions.filter((i) => typeof i?.icon !== "undefined")
                    .length ? (
                  ...
                  <span className="truncate">
                    {selectedOptions.length > 1
                      ? `${selectedOptions.length} انتخاب شده`
          ...
          >
            <Command>
              <CommandInput placeholder="جستجوی مقدار ..." />
              <CommandList>
                <CommandEmpty>مقداری یافت نشد.</CommandEmpty>
                <CommandGroup>
                  {options.map((option) => (
                    <CommandItem
                      key={option.value as string}
        ...
        <Popover open={showValueSelector} onOpenChange={setShowValueSelector}>
          <PopoverTrigger asChild>
            <Button
              ...
              size="sm"
              className={cn(
                "h-10 rounded-none border px-1.5 font-normal dark:bg-input/30",
