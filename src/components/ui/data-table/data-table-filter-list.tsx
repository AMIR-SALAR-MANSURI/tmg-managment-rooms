...
export function DataTableFilterList<TData>({
  ...
  return (
    <Sortable
      ...
      getItemValue={(item) => item.filterId as UniqueIdentifier}
    >
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          <div className="flex flex-col gap-1">
            <p id={labelId} className="font-medium !text-base">
              {filters.length > 0 ? "فیلتر" : "هیچ فیلتری اعمال نشده است"}
            </p>
            <p
              id={descriptionId}
              className={cn(
            ...
            >
              {filters.length > 0
                ? "فیلترها را برای اصلاح ردیف‌هایتان تغییر دهید."
                : "فیلترهایی برای اصلاح ردیف‌هایتان اضافه کنید."}
            </p>
          </div>
          {filters.length > 0 ? (
      ...
      >
        ...
        <Popover open={showFieldSelector} onOpenChange={setShowFieldSelector}>
          <PopoverTrigger asChild>
            <Button
              ...
              variant="ghost"
              size="sm"
              className="justify-between rounded font-normal"
            >
              <span className="truncate">
                {columns.find((column) => column.id === filter.id)?.columnDef
                  .meta?.label ?? "انتخاب فیلد"}
              </span>
          ...
          >
            <Command>
              <CommandInput placeholder="جستجو فیلد ..." />
              <CommandList>
        ...
        >
          <SelectTrigger
            ...
            className="h-8 w-52 lg:w-48 rounded lowercase [&[data-size]]:h-8"
          >
    ...
    case "range": {
      ...
      return (
        <Input
          ...
          placeholder={columnMeta?.placeholder ?? "مقدار را وارد کنید ..."}
        ...
        >
          <FacetedTrigger asChild>
            <Button
              ...
              variant="ghost"
              size="sm"
              className="whitespace-nowrap rounded font-normal"
            >
              <FacetedBadgeList
                options={columnMeta?.options}
                placeholder={columnMeta?.placeholder ?? `انتخاب کنید ...`}
              />
            </Button>
          </FacetedTrigger>
          <FacetedContent
            id={inputListboxId}
            className="w-[200px] min-w-[200px] origin-[var(--radix-popover-content-transform-origin)]"
          >
            <FacetedInput
              aria-label={`Search ${columnMeta?.label} options`}
              placeholder={columnMeta?.placeholder ?? "جستجو کنید ..."}
            />
            <FacetedList>
