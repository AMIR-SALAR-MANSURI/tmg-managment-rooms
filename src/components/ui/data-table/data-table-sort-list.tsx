...
export function DataTableSortList<TData>({
  ...
  const { columnLabels, columns } = React.useMemo(() => {
    ...
    for (const column of table.getAllColumns()) {
      if (!column.columnDef.enableSorting) continue;
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          <div className="flex flex-col gap-1">
            ...
            >
              {sorting.length > 0
                ? "مرتب‌سازی را ویرایش کنید تا ردیف‌ها سامان‌دهی شوند."
                : "مرتب‌سازی اضافه کنید تا ردیف‌ها سامان‌دهی شوند."}
            </p>
          </div>
          {sorting.length > 0 && (
        ...
        <Popover open={showFieldSelector} onOpenChange={setShowFieldSelector}>
          <PopoverTrigger asChild>
            <Button
              ...
              className="w-44 h-8 bg-white border border-border justify-between rounded font-normal"
            >
          ...
          >
            <Command>
              ...
              <CommandList>
                <CommandEmpty>ستون یافت نشد</CommandEmpty>
                <CommandGroup>
                  {columns.map((column) => (
                    <CommandItem
                      key={column.id}
